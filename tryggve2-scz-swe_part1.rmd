...
# Q for KK: do we need birth year etc? Skipped the DOB part now.
```

3.2 Merge SCZ cohort with Cause of Death register 
Add death date to the SCZ cohort, also derive age at death (if deceased)
```{r add death info}
scz.death <- left_join(scz, death, by="id") %>% 
  mutate(death.date = ymd(dod),dod=NULL) %>% # death date as date object
  mutate(age.death = as.integer((death.date - birth.date)/365.25)) # age at death, NAs if not deceased
...
emigrate <- migrate %>%
  filter(migrate.type=="E") %>%
  arrange(id, migrate.date) %>%
  group_by(id) %>%
  mutate(emig.date = first(migrate.date)) %>%
  ungroup()

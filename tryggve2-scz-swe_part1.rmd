...
Added in the exclusion of cases that are predominant bipolar and substance
use disorder.

CASE EXCLUSIONS:

+ Bipolar disorder as the dominant diagnosis (SCZ/SAD ≤ 5 and BIP contacts ≥ 5) 
+ Illicit drug or alcohol dependent predominated (SCZ/SAD ≤ 5 and SUDS contacts ≥ 50) 

```{r extract SCZ cases and create exclusion codes}
scz <- hdr %>%
...
scz <- left_join(scz,bip)
scz <- left_join(scz,suds)

scz <- scz %>%
  mutate(bip=ifelse(is.na(bip),0,bip),
         suds=ifelse(is.na(suds),0,suds),
         case.exclu = case_when((bip >= 5 & nsczcontacts <=5) | (nsczcontacts <= 5 & suds >=50) ~ 1,
                                TRUE ~ 0)) %>%
  select(-bip,-suds) # set case.exclu to 0 to filter out cases w exclusionary dx based on total N scz contacts 
...
### SAVE ALL THE DATA 
```{r save data}
save.image(file = "/Volumes/Common/projects/S3/S3_Research/tryggve2/data/scz.part1.RData",compress = TRUE)
```
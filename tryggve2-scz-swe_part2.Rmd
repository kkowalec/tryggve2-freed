...
int.dis <- hdr %>%
  ...
  slice(1) %>%
  select(id, sui, nintdiscontacts, first.intdis.date)
...
scz.intdis <- inner_join(first.scz.date,int.dis, by="id") %>% # inner join together with scz cases
  mutate(age.first.intdis = as.integer((first.intdis.date - dob)/365.25),
         intdis.childhood = ifelse(age.first.intdis <= 15, 1, 0), # only those with sui dx before age 15 years
         intdis.pre.scz = ifelse(first.intdis.date < first_scz_date, 1, 0)  #  only those with sui dx before scz contact
  ) %>%
...
infect <- infect %>%
  ...
  select(id,infect.childhood,infect.pre.scz,infect.type)

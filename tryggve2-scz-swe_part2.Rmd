...
# for this variable only, we dont care when diagnosed 
...
# extract county,first two digits of SAMS code
sams_code <- sams_code %>%
...
sams_scz <- sams_scz %>%
  ...
  slice(1) %>%
  select(id,SAMSNAMN,REGFAM,GRUPP,Name) # select needed variables
...
sams_scz <- sams_scz %>%
  select(id,Name) %>%
  rename(local_scz_dx=Name)

# link w SCZ files

scz <- left_join(scz,sams_scz)

rm(sams,sams_code,kommun,sams_scz)
```

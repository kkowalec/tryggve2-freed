...
data.f <- "/Users/luyi/CloudStation/WORK_2018/MyProjects_with_Pat/GAPS_MDD/DATA/FREED/DATA/scz.part2.tmp.RData"
...
scz.fhx <- fhx %>%
  ...
  # *WRONG*: if none of the 4 relatives has SCZ (before index person), then fhx = 0   # *CORRECT*:if any of the 4 relatives has no SCZ (Note: after the first condion was applied, this basically grabs those status of 0s or NAs), then fhx = 0
  # otherwise NAs
...
# 2) take the first digit in the 'edlevel' variable, and recategorize


# recategorise
# need birth year and sex from v_indivi
education <- left_join(education,pop, by="id")
education <- education %>%
  mutate(edlevel = case_when(str_detect(edlevel,"^1") ~ "6",
                             str_detect(edlevel,"^2") ~ "9",
                             str_detect(edlevel,"^3") ~ "11",
                             str_detect(edlevel,"^4") ~ "12",
                             str_detect(edlevel,"^5") ~ "14",
                             str_detect(edlevel,"^6|^7") ~ "16",
                                   TRUE ~ NA_character_))
education <- education %>%
  filter(!is.na(edlevel) & !is.na(dob))
### Standardize ed-level by birth year and sex
education <- education %>%
  mutate(birth.yr = substr(dob, 1, 4))
education$edlevel <- as.numeric(education$edlevel)
education$birth.yr <- as.numeric(education$birth.yr)
education$sex <- as.numeric(education$sex)
education$z_edlevel <- rstandard(lm(education$edlevel ~ education$birth.yr + education$sex))
education <- education %>%
  select(id, z_edlevel)
# group with study pop
scz <- left_join(scz, education, by = "id")
rm(education)
```
...
### SAVE ALL THE DATA 
```{r save data}
save.image(file = "/Users/luyi/CloudStation/WORK_2018/MyProjects_with_Pat/GAPS_MDD/DATA/FREED/DATA/scz.part2.tmp.RData",compress = TRUE)
```

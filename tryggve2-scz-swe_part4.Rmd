...
analysis <- left_join(suicide,scz)
...
analysis <- left_join(analysis,suicide, by="id")
...
summary(coxph(Surv(fu.time, outcome_di) ~ sex , data=analysis))$conf.int -> row1 
summary(coxph(Surv(fu.time, outcome_di) ~ age.first.scz , data=analysis))$conf.int -> row2 
summary(coxph(Surv(fu.time, outcome_di) ~ infect.pre.scz, data=analysis))$conf.int -> row3

summary(coxph(Surv(fu.time, outcome_di) ~ psych_pre.scz , data=analysis))$conf.int -> row4

#summary(coxph(Surv(fu.time, outcome_di) ~ std.grade, data=analysis))$conf.int -> row5
summary(coxph(Surv(fu.time, outcome_di) ~ z_edlevel, data=analysis))$conf.int -> row6

summary(coxph(Surv(fu.time, outcome_di) ~ fhx.scz, data=analysis))$conf.int -> row7
summary(coxph(Surv(fu.time, outcome_di) ~ paternal_age, data=analysis))$conf.int -> row8
summary(coxph(Surv(fu.time, outcome_di) ~ maternal_age, data=analysis))$conf.int -> row9
...
cox.multi <- summary(coxph(Surv(fu.time, outcome_di) ~ 
...
fig <- ggplot(data=res) +
        ...
        labs(title = "Predictors of suicide or attempt in SCZ (multiv n=11915, event=1293) ",
...
labels <- c(`0` = "unadjusted (N=13806)", `1` = "adjusted (N=11915, event=1293)")

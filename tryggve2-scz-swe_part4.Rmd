...
### Step 5b: Derive composite outcome for analysis (any of suicide, hsp, death, trs)
```{r}

anyout <- studypop %>%
  arrange(id) %>%
  mutate(end.follow = ymd("2013-12-31")) %>%
  gather("date.outcome1","date.outcome2", "date.outcome3", "date.outcome4","date.outcome5", "dod", "first.emigrate.date","end.follow",
         key = "outcome",
         value = "date") %>%
 # select(id, outcome:date, scz:psych_pre.scz) %>%
  arrange(id,date) %>%
...
# put both unadjusted and adjusted estimates into facets, side by side
labels <- c(`0` = "unadjusted (N=13806)", `1` = "adjusted (N=13157, event=6482)")

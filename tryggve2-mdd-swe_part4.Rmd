...
studypop %>% select_at(ends_with("childhood", "pre.mdd")) %>% skimr::skim()
...
studypop %>% select(paternal_age,maternal_age) %>% skimr::skim()
...
fhx.mdd.wide.2 <- fhx.mdd.wide %>% 
  ...
  mutate(mom.mdd.2 = case_when(mom.mdd == 1 ~ 1, 
         ...
         kid.mdd.2 = case_when(kid.mdd == 1 ~ 1, 
                               ...
                               TRUE ~  NA_real_ ) # i.e., missing kid's info 
         ) %>%
  ...
  mutate(fhx.mdd.2 = case_when(mom.mdd.2==1 | dad.mdd.2==1 | kid.mdd.2==1 | sib.mdd.2==1 ~ 1, # either of immediate family has dx (bf onset of index person)
                         mom.mdd.2==0 & dad.mdd.2==0 & kid.mdd.2==0 & sib.mdd.2==0 ~ 0, # none of immediate family has dx
                         TRUE ~ NA_real_))
...
### Step 3: Examine outcome 1 & 2: suicide attempt or completed suicide
```{r outcome suicide}
outcomes <- studypop %>%
  arrange(id) %>%
  mutate(end.follow = ymd("2014-12-31")) %>%  # maybe change as 2013-12-31
  gather("date.sui.attempt","date.sui.death", "dod", "first.emig.date","end.follow",
         key = "outcome",
         value = "date") %>%
  select(id, sui.attempt:date, mdd:parental.breavement.2) %>%
  arrange(id,date) %>%
  group_by(id) %>%
  mutate(date.outcome.reached = first(date)) %>%
  select(id, date.outcome.reached, sui.attempt:date, first_mdd_date) %>%
  slice(1) 

outcomes <- outcomes %>%
  mutate(time = as.numeric(date - first_mdd_date)/365.25) %>% # calculate time var
...
#outcomes <- outcomes %>%
# mutate(outcome = ifelse(outcome=="end.follow", 0, 1))  <<<- check with KK

outcomes <- outcomes %>%
  mutate(outcome = ifelse(outcome=="date.sui.attempt"|outcome=="date.sui.death", 1, 0))


# merge back w exposures
dataforanalysis <- left_join(outcomes,mdd.perind, by="id")
...
rbind(row2,row3,row4,row5,row6,row7,row8,row9,row10,row11,row12,row13,row14,row17) -> cox.univ  # skip also SEX
...
res$predictors <- factor(res$predictors, levels=rev(c("age.first.mdd",
...
fig <- ggplot(data=res) +
        ...
        labs(title = "Predictors of suicide or attempt in MDD (multiv n=35659, event=4399) ",
...
labels <- c(`0` = "unadjusted (N=130002)", `1` = "adjusted (N=35659, event=4399)")
...
fig2 <-ggplot(data=res) + # take away sex for now
        ...
        geom_hline(yintercept=1, lty=3) +  # add a dotted line at x=1 after flip
        coord_flip() +  # flip coordinates (puts labels on y axis)
        labs(title = "Predictors of suicide or attempt in MDD (birth cohort: 1958 to 1993)",
        x="", y="Hazard Ratio (95% CI)") +
...
ggsave(file="/Users/luyi/CloudStation/WORK_2018/MyProjects_with_Pat/GAPS_MDD/DATA/FREED/fig_sui2.png",fig2, width=8, height=6)
